...
getwd()
chem<-read.table(file = "Yemen.txt", header = TRUE,sep="")
dim1 <- c(chem[1:16,1], "сумма")
...
for (s in c(2,7,9,12,15,16,20)) {
 chem <- read.table(vars[s],header=TRUE)
 for (y in 1:16) {
   for (v in 1:7) {
     alltable [y,v,s] <- chem[y,v+1] 
   }
 }
 for (v in 1:7) {
   alltable[y+1,v,s] <- sum(chem[,v+1])
 }
}
alltable
